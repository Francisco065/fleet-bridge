...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (dataHoje) dataHoje.textContent = new Date().toLocaleDateString('pt-BR', {weekday:'long', year:'numeric', month:'long', day:'numeric'});
...
async function loadOverview() {
  try {
    ...
    const kpiOnlinePct = document.getElementById('kpi-online-pct');
    const distRisco = document.getElementById('dist-risco');
    const lastUpdate = document.getElementById('last-update');

    if (kpiTotal) kpiTotal.textContent = kpis.total_veiculos;
    if (kpiOnline) kpiOnline.textContent = kpis.veiculos_online;
    if (kpiRisco) kpiRisco.textContent = kpis.veiculos_risco;
    if (kpiScore) kpiScore.textContent = kpis.score_medio_frota;

    const pct = kpis.total_veiculos > 0 ?
      Math.round((kpis.veiculos_online / kpis.total_veiculos) * 100) : 0;
    if (kpiOnlinePct) kpiOnlinePct.textContent = pct + '% da frota';
...
async function loadAlertas() {
  try {
    ...
    container.innerHTML = alertas.slice(0, 10).map(a => {
      const sev = a.severity === 'critical' ? 'red' : a.severity === 'high' ? 'orange' : 'yellow';
      const sevColor = sev === 'red' ? '#ef4444' : sev === 'orange' ? '#f97316' : '#f59e0b';
      return '<div class="flex items-start gap-2 p-2.5 rounded-lg cursor-pointer hover:opacity-80 transition" ' +
        'style="background:' + sevColor + '15;border:1px solid ' + sevColor + '30" ' +
        'onclick="marcarLido(' + a.id + ')">' +
        '<i class="fas fa-exclamation-triangle text-xs mt-0.5 flex-shrink-0" style="color:' + sevColor + '"></i>' +
        '<div class="flex-1 min-w-0">' +
          '<p class="text-xs font-medium truncate" style="color:' + sevColor + '">' + (a.placa || 'Veículo ' + a.veiculo_id) + '</p>' +
  ...
  try {
    ...
    chartHoras = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Posições', data: dataTotal, backgroundColor: 'rgba(59,130,246,0.3)', borderColor: '#3b82f6', borderWidth: 1, borderRadius: 2 },
          { label: 'Alertas', data: dataAlertas, backgroundColor: 'rgba(239,68,68,0.3)', borderColor: '#ef4444', borderWidth: 1, borderRadius: 2 }
        ]
      },
      options: {
        responsive: true,
...
async function loadMapaPosicoes() {
  ...
  try {
    ...
    veiculos.forEach(v => {
      ...
      const pulseClass = v.status_online ? 'pulse' : '';
      const html = '<div class="vehicle-marker ' + pulseClass + '" style="background:' + cor + '30;border-color:' + cor + '">' +
        '<i class="fas fa-car" style="color:' + cor + ';font-size:11px"></i>' +
      '</div>';

      const icon = L.divIcon({ html: html, className: '', iconSize: [34, 34], iconAnchor: [17, 17] });
      const marker = L.marker([v.latitude, v.longitude], { icon: icon });

      const popupHtml = '<div style="background:#1e293b;color:#f1f5f9;border-radius:10px;padding:12px;min-width:200px;border:1px solid rgba(148,163,184,0.2)">' +
...
async function loadIndicadores() {
  try {
    ...
    if (veiculos.length === 0) {
      container.innerHTML = '<div class="text-slate-500 text-sm text-center py-10 col-span-3">' +
        '<i class="fas fa-car mr-2"></i>Nenhum veículo cadastrado.<br><br>' +
        '<button onclick="showModal(\'modal-sync\')" class="btn-primary">Sincronizar Veículos</button>' +
      '</div>';
      return;
    }
...
async function loadRanking(tipo, btnEl) {
  ...
  try {
    ...
    body.innerHTML = ranking.map((v, i) => {
      ...
      let metricCell = '';
      if (tipo === 'risco') {
        metricCell = '<td class="p-4 text-sm text-slate-300 hidden md:table-cell">' + Math.round(v.pico_velocidade||0) + ' km/h</td>';
      } else if (tipo === 'velocidade') {
        metricCell = '<td class="p-4 text-sm text-slate-300 hidden md:table-cell">' + Math.round(v.pico_velocidade||0) + ' km/h</td>';
      } else {
        metricCell = '<td class="p-4 text-sm text-slate-300 hidden md:table-cell">' + (v.minutos_ocioso||0) + ' min</td>';
      }
    ...
    if (body) body.innerHTML = '<tr><td colspan="6" class="text-center p-8 text-slate-500 text-xs">Erro ao carregar ranking</td></tr>';
  }
}
  ...
  try {
    ...
    body.innerHTML = logs.map(l => {
      ...
      return '<tr class="table-row border-b border-slate-800/50">' +
        ...
        '<td class="p-4">' +
          '<span class="badge" style="color:' + cor + ';background:' + cor + '20">' + statusLabel + '</span>' +
        '</td>' +
...
async function coletarDados() {
  ...
  try {
    ...
    if (txt) txt.textContent = r.ok ? (r.posicoes || 0) + ' pos.' : 'Erro';
...
async function testarConexao() {
  const username = document.getElementById('cfg-username') ? document.getElementById('cfg-username').value : '';
  const password = document.getElementById('cfg-password') ? document.getElementById('cfg-password').value : '';
  const appid = document.getElementById('cfg-appid') ? document.getElementById('cfg-appid').value : 'portal';
  ...
  try {
    ...
    showCfgResult((err.response && err.response.data && err.response.data.message) || 'Erro de conexão', 'error');
  }
}
...
async function testarEConectarModal() {
  const username = document.getElementById('m-username') ? document.getElementById('m-username').value : '';
  const password = document.getElementById('m-password') ? document.getElementById('m-password').value : '';
  const appid = document.getElementById('m-appid') ? document.getElementById('m-appid').value : 'portal';
  ...
  try {
    ...
    if (resultEl) {
      ...
      resultEl.textContent = '&#10007; ' + ((err.response && err.response.data && err.response.data.message) || 'Falha na conexão');
    }
  }
}
...
async function sincronizarVeiculos() {
  ...
  try {
    ...
    showToast('Erro: ' + ((err.response && err.response.data && err.response.data.error) || 'Falha'), 'error');
  }
}
...
async function syncAndShow(tipo) {
  ...
  try {
    ...
    if (resultEl) {
      resultEl.className = 'text-xs p-3 rounded-lg bg-red-500/20 text-red-400 border border-red-500/30';
      resultEl.textContent = '&#10007; ' + ((err.response && err.response.data && err.response.data.error) || 'Erro');
    }
  }
}
...
function startAutoRefresh() {
  ...
  autoRefreshTimer = setInterval(async () => {
    ...
    if (activeSectionId === 'sec-torre') {
      await loadOverview();
      await loadTimeline();
      await loadAlertas();
      await loadStatsHora();
    } else if (activeSectionId === 'sec-mapa') {
      loadMapaPosicoes();
    } else if (activeSectionId === 'sec-indicadores') {
      loadIndicadores();
    }
